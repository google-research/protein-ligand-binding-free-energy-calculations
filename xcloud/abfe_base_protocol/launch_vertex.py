...
_BASE_IMAGE = flags.DEFINE_string(
    'base_image', None, 'Docker image with Gromacs.', short_name='i'
)
...
_MDP_PATH = flags.DEFINE_string(
    'mdp_path', None, 'Path to MDP files.', short_name='m'
)
_OUT_PATH = flags.DEFINE_string(
    'out_path',
    None,
    'Output directory for all calculations (in /gcs/bucket/directory format).',
    short_name='o',
)
_LIG_IDS = flags.DEFINE_string(
    'lig_ids',
    None,
    (
        'Ligand IDs for which to run ABFE calculations. This can be a'
        ' comma-separated list (e.g., "0,1,4") or an inclusive range (e.g.,'
        ' "0-9"). We expect the folders {top_path}/ligand_{lig_id} to exists.'
    ),
)
_APO_DIR = flags.DEFINE_string(
    'apo_dir',
    'apo_protein',
    'Name of directory with TOP/GRO files for apo protein simulations.',
)
...
def _parse_lig_ids(lig_ids_str: str) -> List:
  if '-' in lig_ids_str:  # Assume range.
    ...
    if sum(1 for _ in lig_ids) < 1:
      raise ValueError(
          f'--lig_ids input "{lig_ids_str}" does not specify a range correctly'
      )
